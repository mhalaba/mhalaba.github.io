# Zdjęcia — pochodzenie i licencje

## Uzupełnione we wrześniu 2026

Dziewięć modeli miało wcześniej przypisane zdjęcie innego bezzałogowca. Poniższe
pliki pochodzą z Wikimedia Commons, mają wskazaną licencję i zostały przeskalowane
do 1200 px szerokości.

| Model | Plik | Licencja | Autor / źródło |
| --- | --- | --- | --- |
| Pekło | `drones/peklo.jpg` | CC BY 4.0 | Ministerstwo Przemysłu Strategicznego Ukrainy |
| LUCAS (FLM-136) | `drones/lucas.jpg` | domena publiczna | U.S. Department of Defense (VIRIN 251123-D-D0477-1031) |
| FP-5 Flamingo | `drones/flamingo.jpg` | CC BY 4.0 | Wikimedia Commons, MSPO 2026 Kielce |
| A1-CM Furia | `drones/furia.jpg` | CC BY 4.0 | 24. Samodzielna Brygada Zmechanizowana im. Króla Daniela |
| Ukrspecsystems PD-2 | `drones/pd2.jpg` | CC BY-SA 4.0 | Wikimedia Commons, użytkownik DoroshenkoE |
| FPV światłowodowe | `drones/fpv-fiber.jpg` | CC BY 4.0 | АрміяІнформ |
| Merops | `drones/merops.jpg` | domena publiczna | Sgt. Luis Garcia, 10th Army Air and Missile Defense Command, U.S. Army |
| Sting (Wild Hornets) | `drones/sting.jpg` | CC BY 4.0 | Sztab Generalny Sił Zbrojnych Ukrainy |
| Hero-120 | `drones/hero120.jpg` | CC BY-SA 4.0 | Wikimedia Commons, użytkownik MilleniumAC |
| Geran-4 | `drones/geran4.jpg` | CC0 | Wikimedia Commons, autor nieznany |

Zdjęcie Hero-120 przedstawia stoisko UVision z trzema pociskami krążącymi;
Hero-120 stoi pośrodku, opisany tabliczką, obok Hero-30 i Hero-400EC.

## Wciąż współdzielone

Dla tych modeli nie znaleziono zdjęcia na otwartej licencji. Każdy używa zdjęcia
pokrewnego modelu — to nadal błąd, tylko w obrębie jednego kraju i zbliżonej klasy:

| Model | Używa zdjęcia | Uwaga |
| --- | --- | --- |
| Luckyshot | `fpv.jpg` (FPV 10-calowy) | zdjęcie klasowe, nie konkretnego modelu |
| Lucky Strike 15 | `vampire2.jpg` (Vampire) | inny producent, ta sama klasa bombowca |
| Fire Point FP-2 | `fp1.jpg` (FP-1) | ten sam producent, inny model |
| AQ-400 Scythe | `fp1.jpg` (FP-1) | inny producent |
| WIY Spys | `strila.jpg` (WIY Striła) | ten sam producent, model siostrzany |

## Zaślepka zamiast zdjęcia

Geran-5 (`drones/geran5.svg`) nie ma zdjęcia — na Wikimedia Commons nie ma
żadnego wizerunku tej konstrukcji na wolnej licencji. Zamiast podstawiać
zdjęcie irańskiego Karrara, na którym Geran-5 jest wzorowany, albo Gerana-4,
karta dostała rysunek poglądowy klasy z wyraźnym podpisem, że nie jest to
wizerunek egzemplarza. Do podmiany, gdy pojawi się zdjęcie z licencją.

Gerbera-2 (`drones/gerbera2.svg`) — to samo. Wersja z sierpnia 2026 r. nie ma
zdjęcia na wolnej licencji, a zdjęcia zwykłej Gerbery pokazują mniejszy,
inaczej wyglądający płatowiec, więc byłyby mylące.

## Pozostałe zdjęcia

Reszta katalogu pochodzi ze źródeł otwartych zebranych przy budowie atlasu;
poszczególne pliki nie mają zapisanego osobno autorstwa.
